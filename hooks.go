	...
	/*
		...
		if err != nil {
			...
			case exitUsageError:
				...
				if !ok {
					logError("Usage() for plugin is not a goenable5.UsageFunc: %T", usageSym)
					return
				}
				logError("Usage:\n%s", usage())
...
func Unload() {
	for name, p := range importCache {
		...
		if !ok {
			logError("Unload() for plugin '%s' is not a goenable5.UnloadFunc: %T", name, unloadSym)
			continue
		}
		err = unload()
		if err != nil {
...
func run(args []string) (*plugin.Plugin, int, error) {
	if len(args) == 0 {
		return nil, exitUsageError, fmt.Errorf("Usage: goenable5 help|load|run [args ...]")
	}
	if args[0] == "help" {
	...
	case "run":
		...
		case RunErrFunc:
			...
			return p, returnCode, err
		default:
			return p, exitGeneralError, fmt.Errorf("Run() for plugin is not a goenable5.Run*Func: Run = %T", runSym)
		}
	case "test1":
		pp.Println(`test1...................................`)
	case "load":
...
func tryLoad(p *plugin.Plugin) error {
	...
	if !ok {
		return fmt.Errorf("Load() for plugin is not a goenable5.LoadFunc: Load = %T", loadSym)
	}
	return load()
}
